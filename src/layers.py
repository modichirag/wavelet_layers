...
class InvertibleLinear(nn.Module):
    ...
    def setup(self):
        ...
        rngp, rngl, rngs, rngu = rng[1:]

        W = orthogonal()(rng[0], (self.input_dim, self.input_dim))
        P, L, U = jscipy.linalg.lu(W)
    ...
    def _forward(self, x):
        ...
        U = jnp.triu(self.U, 1)
        W = self.P @ self.L @ (U + jnp.diag(self.S))
        outputs = x @ W
    ...
    def __call__(self, x, reverse=False, ldj=None, rng=None):
...
class WaveletLayer(nn.Module):
    ...
    def _forward(self, x):
        ...
        for i, detail in enumerate(details):
            ...
            logdetjac += jnp.sum(jnp.log(self.renorm(wb[..., :3])), axis=[1, 2, 3])
            h, v, d = detail
            h = h*wh + bh
            v = v*wv + bv
            d = d*wd + bd
...
class WaveletLayer_sadapt(nn.Module):
    ...
    def setup(self):
        ...
        '''
        self.networks = [GatedConvNet(c_hidden=self.c_hidden, c_out=6, num_layers=2)  for _ in range(self.L)]
        self.s_flow = InvertibleLinear(int(self.D/2**self.L)**2, key=self.key)
        assert self.L == len(self.networks)
