...
class Camels_dataloader():
    ...
    def __post_init__(self ):
        ...
        data = np.load(f'/mnt/ceph/users/cmodi/ML_data/camels/{self.obs}_TNG_{self.d}.npy') 
        data /= data.std() #normalize
        if self.logit : 
            data = np.log(data)
        data = np.expand_dims(data, axis=-1)
        targets = np.load(f'/mnt/ceph/users/cmodi/ML_data/camels/zeros.npy') 
        transform = None 
