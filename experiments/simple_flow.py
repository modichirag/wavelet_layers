...
def create_simple_flow(use_vardeq=True):
    ...
    for i in range(n_flow_layers):
        flow_layers += [CouplingLayer(network=GatedConvNet(c_out=2, c_hidden=c_hidden),
                                      mask=create_checkerboard_mask(h=32, w=32, invert=(i%2==1)),
                                      c_in=1)]
...
#####
model = create_simple_flow(use_vardeq=True)
...
ckpt_path = '/mnt/ceph/users/cmodi/wavelet_layers/mnist/simpleflow/ckpt/'
fig_path = f'{ckpt_path}/figs/'
os.makedirs(fig_path, exist_ok=True)

state = train_state.TrainState.create(apply_fn=model.apply, params=params, tx=optimizer)
trainer = TrainerModule('simple', state, model, nckpt=1, ckpt_path=ckpt_path)
